...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-route53-recovery-cluster.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-cluster.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-cluster.dev",
			expectURL:     aws.String("https://env-route53-recovery-cluster.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-route53-recovery-cluster.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-cluster.dev",
			expectURL:     aws.String("http://config-route53-recovery-cluster.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-route53-recovery-cluster.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-route53-recovery-cluster.dev",
			clientEndpoint: aws.String("https://client-route53-recovery-cluster.dev"),
			expectURL:      aws.String("https://client-route53-recovery-cluster.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ROUTE53_RECOVERY_CLUSTER", c.envService)
			}
