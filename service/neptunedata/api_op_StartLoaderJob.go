...
import (
	...
	"github.com/mniehe/aws-sdk-go-v2/service/neptunedata/types"
...
type StartLoaderJobInput struct {
	...
	// folder that is specified. The URI can be in any of the following formats.
	...
	// and/or folders. The specified folder or folders can contain multiple vertex
	// files and multiple edge files.
	//
	// This member is required.
	Source *string

	// This is an optional parameter that can make a queued load request contingent on
	// the successful completion of one or more previous jobs in the queue. Neptune can
	// queue up as many as 64 load requests at a time, if their queueRequest
	// parameters are set to "TRUE" . The dependencies parameter lets you make
	// execution of such a queued request dependent on the successful completion of one
	// or more specified previous requests in the queue. For example, if load Job-A
	// and Job-B are independent of each other, but load Job-C needs Job-A and Job-B
	...
	// LOAD_FAILED_BECAUSE_DEPENDENCY_NOT_SATISFIED . You can set up multiple levels of
	// dependency in this way, so that the failure of one job will cause all requests
	...
	// "TRUE" , "FALSE" . Default value: "TRUE" . When this parameter is set to "FALSE"
	// , the loader tries to load all the data in the location specified, skipping any
	// entries with errors. When this parameter is set to "TRUE" , the loader stops as
	...
	//   found, the loader stops. The loader avoids reloading files that were
	...
	//   source successfully, nothing is reloaded, and the loader returns success.
	...
	//   source, and if it finds one, resumes that job, just as in RESUME mode. If the
	//   loader doesn't find a previous load job from the same source, it loads all data
	//   from the source, just as in NEW mode.
	Mode types.Mode

	// The optional parallelism parameter can be set to reduce the number of threads
	// used by the bulk load process. Allowed values:
	...
	//   resources. This does not mean, however, that the OVERSUBSCRIBE setting results
	//   in 100% CPU utilization. Because the load operation is I/O bound, the highest
	//   CPU utilization to expect is in the 60% to 70% range.
	// Default value: HIGH The parallelism setting can sometimes result in a deadlock
	// between threads when loading openCypher data. When this happens, Neptune returns
	// the LOAD_DATA_DEADLOCK error. You can generally fix the issue by setting
	...
	//   specified (for non-quads formats and NQUAD entries with no graph). The default
	//   is https://aws.amazon.com/neptune/vocab/v01/DefaultNamedGraph .
	...
	//   treated as nulls and are not loaded. If allowEmptyStrings is set to true , the
	...
	// properties. This is not supported for loading openCypher data. Allowed values:
	// "TRUE" , "FALSE" . Default value: "FALSE" . By default, or when
	// updateSingleCardinalityProperties is explicitly set to "FALSE" , the loader
	// treats a new value as an error, because it violates single cardinality. When
	// updateSingleCardinalityProperties is set to "TRUE" , on the other hand, the bulk
	// loader replaces the existing value with the new one. If multiple edge or
	...
	// userProvidedEdgeIds is absent or set to True , an :ID column must be present in
	// every relationship file in the load. When userProvidedEdgeIds is present and
	// set to False , relationship files in the load must not contain an :ID column.
	// Instead, the Neptune loader automatically generates an ID for each relationship.
	// It's useful to provide relationship IDs explicitly so that the loader can resume
	// loading after error in the CSV data have been fixed, without having to reload
	// any relationships that have already been loaded. If relationship IDs have not
	// been explicitly assigned, the loader cannot resume a failed load if any
