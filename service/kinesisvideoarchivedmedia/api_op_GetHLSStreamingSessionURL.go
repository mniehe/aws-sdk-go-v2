...
//     retrieve stream media. Data retrieved with this action is billable. For more
//     information, see Kinesis Video Streams pricing (https://aws.amazon.com/kinesis/video-streams/pricing/)
//     .
//
...
// monitoring the GetMP4MediaFragment.OutgoingBytes Amazon CloudWatch metric. For
...
type GetHLSStreamingSessionURLInput struct {
	...
	// GetMP4MediaFragment , or GetTSFragment can be made for that session. The
	...
	// playback. We recommend that a live HLS media playlist have a minimum of 3
	...
	//   fragments are included in an HLS media playlist, even if there is a gap between
	...
	//   not added to the HLS media playlist if they are older than the newest fragment
	//   in the playlist. If the missing fragment becomes available after a subsequent
	//   fragment is added to the playlist, the older fragment is not added, and the gap
	//   is not filled.
