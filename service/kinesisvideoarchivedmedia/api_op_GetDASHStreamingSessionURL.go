...
import (
	...
	"github.com/mniehe/aws-sdk-go-v2/service/kinesisvideoarchivedmedia/types"
...
// operation. An Amazon Kinesis video stream has the following requirements for
...
//     MPEG-DASH). Don't share or store this token where an unauthorized entity can
//     access it. The token provides access to the content of the stream. Safeguard the
//     token with the same measures that you use with your Amazon Web Services
//     credentials. The media that is made available through the manifest consists only
//     of the requested stream, time range, and format. No other media data (such as
//     frames outside the requested window or alternate bitrates) is made available.
...
type GetDASHStreamingSessionURLInput struct {
	...
	// calls to GetDashManifest , GetMP4InitFragment , or GetMP4MediaFragment can be
	// made for that session. The default is 300 (5 minutes).
	Expires *int32
	...
	// returned, up to this maximum number. When there are a higher number of fragments
	...
	// latency, but it decreases the likelihood that rebuffering will occur during
	// playback. We recommend that a live MPEG-DASH manifest have a minimum of 3
	// fragments and a maximum of 10 fragments. The default is 5 fragments if
	// PlaybackMode is LIVE or LIVE_REPLAY , and 1,000 if PlaybackMode is ON_DEMAND .
	...
	//   position in the playback window to display. In LIVE mode, the newest available
	//   fragments are included in an MPEG-DASH manifest, even if there is a gap between
	//   fragments (that is, if a fragment is missing). A gap like this might cause a
	//   media player to halt or cause a jump in playback. In this mode, fragments are
	//   not added to the MPEG-DASH manifest if they are older than the newest fragment
	//   in the playlist. If the missing fragment becomes available after a subsequent
	//   fragment is added to the manifest, the older fragment is not added, and the gap
	//   is not filled.
