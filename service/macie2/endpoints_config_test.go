...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-macie2.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-macie2.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-macie2.dev",
			expectURL:     aws.String("https://env-macie2.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-macie2.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-macie2.dev",
			expectURL:     aws.String("http://config-macie2.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-macie2.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-macie2.dev",
			clientEndpoint: aws.String("https://client-macie2.dev"),
			expectURL:      aws.String("https://client-macie2.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_MACIE2", c.envService)
			}
