...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-eks-auth.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-eks-auth.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-eks-auth.dev",
			expectURL:     aws.String("https://env-eks-auth.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-eks-auth.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-eks-auth.dev",
			expectURL:     aws.String("http://config-eks-auth.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-eks-auth.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-eks-auth.dev",
			clientEndpoint: aws.String("https://client-eks-auth.dev"),
			expectURL:      aws.String("https://client-eks-auth.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_EKS_AUTH", c.envService)
			}
