...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-cognito-identity-provider.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cognito-identity-provider.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cognito-identity-provider.dev",
			expectURL:     aws.String("https://env-cognito-identity-provider.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-cognito-identity-provider.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cognito-identity-provider.dev",
			expectURL:     aws.String("http://config-cognito-identity-provider.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-cognito-identity-provider.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-cognito-identity-provider.dev",
			clientEndpoint: aws.String("https://client-cognito-identity-provider.dev"),
			expectURL:      aws.String("https://client-cognito-identity-provider.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_COGNITO_IDENTITY_PROVIDER", c.envService)
			}
