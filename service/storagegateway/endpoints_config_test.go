...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-storage-gateway.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-storage-gateway.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-storage-gateway.dev",
			expectURL:     aws.String("https://env-storage-gateway.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-storage-gateway.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-storage-gateway.dev",
			expectURL:     aws.String("http://config-storage-gateway.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-storage-gateway.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-storage-gateway.dev",
			clientEndpoint: aws.String("https://client-storage-gateway.dev"),
			expectURL:      aws.String("https://client-storage-gateway.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_STORAGE_GATEWAY", c.envService)
			}
