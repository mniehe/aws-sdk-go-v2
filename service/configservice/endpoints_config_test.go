...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-config-service.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-config-service.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-config-service.dev",
			expectURL:     aws.String("https://env-config-service.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-config-service.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-config-service.dev",
			expectURL:     aws.String("http://config-config-service.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-config-service.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-config-service.dev",
			clientEndpoint: aws.String("https://client-config-service.dev"),
			expectURL:      aws.String("https://client-config-service.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_CONFIG_SERVICE", c.envService)
			}
