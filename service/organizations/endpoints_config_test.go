...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-organizations.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-organizations.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-organizations.dev",
			expectURL:     aws.String("https://env-organizations.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-organizations.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-organizations.dev",
			expectURL:     aws.String("http://config-organizations.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-organizations.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-organizations.dev",
			clientEndpoint: aws.String("https://client-organizations.dev"),
			expectURL:      aws.String("https://client-organizations.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ORGANIZATIONS", c.envService)
			}
