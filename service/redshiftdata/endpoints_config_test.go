...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-redshift-data.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-data.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-data.dev",
			expectURL:     aws.String("https://env-redshift-data.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-redshift-data.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-data.dev",
			expectURL:     aws.String("http://config-redshift-data.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-redshift-data.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-redshift-data.dev",
			clientEndpoint: aws.String("https://client-redshift-data.dev"),
			expectURL:      aws.String("https://client-redshift-data.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_REDSHIFT_DATA", c.envService)
			}
