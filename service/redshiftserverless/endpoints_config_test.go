...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-redshift-serverless.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-serverless.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-serverless.dev",
			expectURL:     aws.String("https://env-redshift-serverless.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-redshift-serverless.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-redshift-serverless.dev",
			expectURL:     aws.String("http://config-redshift-serverless.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-redshift-serverless.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-redshift-serverless.dev",
			clientEndpoint: aws.String("https://client-redshift-serverless.dev"),
			expectURL:      aws.String("https://client-redshift-serverless.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_REDSHIFT_SERVERLESS", c.envService)
			}
