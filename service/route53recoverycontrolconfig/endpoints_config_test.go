...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-route53-recovery-control-config.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-control-config.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-control-config.dev",
			expectURL:     aws.String("https://env-route53-recovery-control-config.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-route53-recovery-control-config.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route53-recovery-control-config.dev",
			expectURL:     aws.String("http://config-route53-recovery-control-config.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-route53-recovery-control-config.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-route53-recovery-control-config.dev",
			clientEndpoint: aws.String("https://client-route53-recovery-control-config.dev"),
			expectURL:      aws.String("https://client-route53-recovery-control-config.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ROUTE53_RECOVERY_CONTROL_CONFIG", c.envService)
			}
