...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-application-discovery-service.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-application-discovery-service.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-application-discovery-service.dev",
			expectURL:     aws.String("https://env-application-discovery-service.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-application-discovery-service.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-application-discovery-service.dev",
			expectURL:     aws.String("http://config-application-discovery-service.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-application-discovery-service.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-application-discovery-service.dev",
			clientEndpoint: aws.String("https://client-application-discovery-service.dev"),
			expectURL:      aws.String("https://client-application-discovery-service.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_APPLICATION_DISCOVERY_SERVICE", c.envService)
			}
