...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-customer-profiles.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-customer-profiles.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-customer-profiles.dev",
			expectURL:     aws.String("https://env-customer-profiles.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-customer-profiles.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-customer-profiles.dev",
			expectURL:     aws.String("http://config-customer-profiles.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-customer-profiles.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-customer-profiles.dev",
			clientEndpoint: aws.String("https://client-customer-profiles.dev"),
			expectURL:      aws.String("https://client-customer-profiles.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_CUSTOMER_PROFILES", c.envService)
			}
