...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-ecr.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ecr.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ecr.dev",
			expectURL:     aws.String("https://env-ecr.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-ecr.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ecr.dev",
			expectURL:     aws.String("http://config-ecr.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-ecr.dev",
			...
			expectURL:      aws.String("https://client-ecr.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ECR", c.envService)
			}
