...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-migration-hub-refactor-spaces.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub-refactor-spaces.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub-refactor-spaces.dev",
			expectURL:     aws.String("https://env-migration-hub-refactor-spaces.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-migration-hub-refactor-spaces.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub-refactor-spaces.dev",
			expectURL:     aws.String("http://config-migration-hub-refactor-spaces.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-migration-hub-refactor-spaces.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-migration-hub-refactor-spaces.dev",
			clientEndpoint: aws.String("https://client-migration-hub-refactor-spaces.dev"),
			expectURL:      aws.String("https://client-migration-hub-refactor-spaces.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_MIGRATION_HUB_REFACTOR_SPACES", c.envService)
			}
