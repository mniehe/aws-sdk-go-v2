...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-migrationhub-config.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migrationhub-config.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migrationhub-config.dev",
			expectURL:     aws.String("https://env-migrationhub-config.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-migrationhub-config.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migrationhub-config.dev",
			expectURL:     aws.String("http://config-migrationhub-config.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-migrationhub-config.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-migrationhub-config.dev",
			clientEndpoint: aws.String("https://client-migrationhub-config.dev"),
			expectURL:      aws.String("https://client-migrationhub-config.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_MIGRATIONHUB_CONFIG", c.envService)
			}
