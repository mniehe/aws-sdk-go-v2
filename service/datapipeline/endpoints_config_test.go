...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-data-pipeline.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-data-pipeline.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-data-pipeline.dev",
			expectURL:     aws.String("https://env-data-pipeline.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-data-pipeline.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-data-pipeline.dev",
			expectURL:     aws.String("http://config-data-pipeline.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-data-pipeline.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-data-pipeline.dev",
			clientEndpoint: aws.String("https://client-data-pipeline.dev"),
			expectURL:      aws.String("https://client-data-pipeline.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_DATA_PIPELINE", c.envService)
			}
