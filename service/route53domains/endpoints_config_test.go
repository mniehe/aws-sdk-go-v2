...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-route-53-domains.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53-domains.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53-domains.dev",
			expectURL:     aws.String("https://env-route-53-domains.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-route-53-domains.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53-domains.dev",
			expectURL:     aws.String("http://config-route-53-domains.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-route-53-domains.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-route-53-domains.dev",
			clientEndpoint: aws.String("https://client-route-53-domains.dev"),
			expectURL:      aws.String("https://client-route-53-domains.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ROUTE_53_DOMAINS", c.envService)
			}
