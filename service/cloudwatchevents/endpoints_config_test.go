...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-cloudwatch-events.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudwatch-events.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudwatch-events.dev",
			expectURL:     aws.String("https://env-cloudwatch-events.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-cloudwatch-events.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudwatch-events.dev",
			expectURL:     aws.String("http://config-cloudwatch-events.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-cloudwatch-events.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-cloudwatch-events.dev",
			clientEndpoint: aws.String("https://client-cloudwatch-events.dev"),
			expectURL:      aws.String("https://client-cloudwatch-events.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_CLOUDWATCH_EVENTS", c.envService)
			}
