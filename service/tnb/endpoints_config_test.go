...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-tnb.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-tnb.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-tnb.dev",
			expectURL:     aws.String("https://env-tnb.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-tnb.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-tnb.dev",
			expectURL:     aws.String("http://config-tnb.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-tnb.dev",
			...
			expectURL:      aws.String("https://client-tnb.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_TNB", c.envService)
			}
