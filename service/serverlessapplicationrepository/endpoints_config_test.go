...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-serverlessapplicationrepository.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-serverlessapplicationrepository.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-serverlessapplicationrepository.dev",
			expectURL:     aws.String("https://env-serverlessapplicationrepository.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-serverlessapplicationrepository.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-serverlessapplicationrepository.dev",
			expectURL:     aws.String("http://config-serverlessapplicationrepository.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-serverlessapplicationrepository.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-serverlessapplicationrepository.dev",
			clientEndpoint: aws.String("https://client-serverlessapplicationrepository.dev"),
			expectURL:      aws.String("https://client-serverlessapplicationrepository.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_SERVERLESSAPPLICATIONREPOSITORY", c.envService)
			}
