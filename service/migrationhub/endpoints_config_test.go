...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-migration-hub.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub.dev",
			expectURL:     aws.String("https://env-migration-hub.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-migration-hub.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-migration-hub.dev",
			expectURL:     aws.String("http://config-migration-hub.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-migration-hub.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-migration-hub.dev",
			clientEndpoint: aws.String("https://client-migration-hub.dev"),
			expectURL:      aws.String("https://client-migration-hub.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_MIGRATION_HUB", c.envService)
			}
