...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-cloudhsm.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudhsm.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudhsm.dev",
			expectURL:     aws.String("https://env-cloudhsm.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-cloudhsm.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-cloudhsm.dev",
			expectURL:     aws.String("http://config-cloudhsm.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-cloudhsm.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-cloudhsm.dev",
			clientEndpoint: aws.String("https://client-cloudhsm.dev"),
			expectURL:      aws.String("https://client-cloudhsm.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_CLOUDHSM", c.envService)
			}
