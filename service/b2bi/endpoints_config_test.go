...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-b2bi.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-b2bi.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-b2bi.dev",
			expectURL:     aws.String("https://env-b2bi.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-b2bi.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-b2bi.dev",
			expectURL:     aws.String("http://config-b2bi.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-b2bi.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-b2bi.dev",
			clientEndpoint: aws.String("https://client-b2bi.dev"),
			expectURL:      aws.String("https://client-b2bi.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_B2BI", c.envService)
			}
