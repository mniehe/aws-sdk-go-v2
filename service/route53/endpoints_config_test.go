...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-route-53.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53.dev",
			expectURL:     aws.String("https://env-route-53.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-route-53.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-route-53.dev",
			expectURL:     aws.String("http://config-route-53.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-route-53.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-route-53.dev",
			clientEndpoint: aws.String("https://client-route-53.dev"),
			expectURL:      aws.String("https://client-route-53.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_ROUTE_53", c.envService)
			}
