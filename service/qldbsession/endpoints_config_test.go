...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-qldb-session.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-qldb-session.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-qldb-session.dev",
			expectURL:     aws.String("https://env-qldb-session.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-qldb-session.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-qldb-session.dev",
			expectURL:     aws.String("http://config-qldb-session.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-qldb-session.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-qldb-session.dev",
			clientEndpoint: aws.String("https://client-qldb-session.dev"),
			expectURL:      aws.String("https://client-qldb-session.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_QLDB_SESSION", c.envService)
			}
