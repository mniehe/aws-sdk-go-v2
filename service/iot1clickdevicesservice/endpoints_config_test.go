...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-iot-1click-devices-service.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-iot-1click-devices-service.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-iot-1click-devices-service.dev",
			expectURL:     aws.String("https://env-iot-1click-devices-service.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-iot-1click-devices-service.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-iot-1click-devices-service.dev",
			expectURL:     aws.String("http://config-iot-1click-devices-service.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-iot-1click-devices-service.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-iot-1click-devices-service.dev",
			clientEndpoint: aws.String("https://client-iot-1click-devices-service.dev"),
			expectURL:      aws.String("https://client-iot-1click-devices-service.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_IOT_1CLICK_DEVICES_SERVICE", c.envService)
			}
