...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-kafka.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-kafka.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-kafka.dev",
			expectURL:     aws.String("https://env-kafka.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-kafka.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-kafka.dev",
			expectURL:     aws.String("http://config-kafka.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-kafka.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-kafka.dev",
			clientEndpoint: aws.String("https://client-kafka.dev"),
			expectURL:      aws.String("https://client-kafka.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_KAFKA", c.envService)
			}
