...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-pi.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-pi.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-pi.dev",
			expectURL:     aws.String("https://env-pi.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-pi.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-pi.dev",
			expectURL:     aws.String("http://config-pi.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-pi.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-pi.dev",
			clientEndpoint: aws.String("https://client-pi.dev"),
			expectURL:      aws.String("https://client-pi.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_PI", c.envService)
			}
