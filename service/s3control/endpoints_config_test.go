...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-s3-control.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3-control.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3-control.dev",
			expectURL:     aws.String("https://env-s3-control.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-s3-control.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3-control.dev",
			expectURL:     aws.String("http://config-s3-control.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-s3-control.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-s3-control.dev",
			clientEndpoint: aws.String("https://client-s3-control.dev"),
			expectURL:      aws.String("https://client-s3-control.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_S3_CONTROL", c.envService)
			}
